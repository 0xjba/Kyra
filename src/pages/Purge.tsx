...
import { revealInFinder, pickFolder, type ArtifactEntry } from "../lib/tauri";
...
const RECENT_PATHS_KEY = "kyra_purge_recent_paths";
  ...
  return (
    <div className="centered">
      ...
      <div className="purge-path-row">
        <div className="purge-path-input-wrap">
          <FolderOpen
            size={14}
            className="purge-path-icon clickable"
      ...
      {/* Quick-pick chips */}
      <div className="purge-quick-picks">
        {quickPaths.map((p) => (
          <button
            key={p}
            className={`purge-pick-chip${rootPath === p ? " active" : ""}`}
      ...
      {/* What gets detected */}
      <div className="purge-detected-section">
        <span className="purge-detected-label">WHAT GETS DETECTED</span>
        <div className="purge-detected-types">
          {DETECTED_TYPES.map((t) => (
            <span key={t.label} className="purge-detected-chip">
              <span className="purge-detected-dot" style={{ backgroundColor: t.color }} />
              {t.label}
            </span>
          ))}
        </div>
...
/* ── Storage Bar ── */
function StorageBar({ categories }: { categories: [string, ArtifactEntry[]][] }) {
  ...
  for (const [type, items] of categories) {
    ...
    if (size > 0) {
      segments.push({ name: type, size, color: getTypeColor(type) });
    }
  }
  ...
  return (
    <div className="purge-storage-bar">
      <div className="purge-storage-track">
        {segments.map((seg, i) => {
          const pct = (seg.size / totalSize) * 100;
          return (
            <div
              key={seg.name}
              className="purge-storage-segment"
              style={{
      ...
      <div className="purge-storage-legend">
        {segments.map((seg) => (
          <div key={seg.name} className="purge-storage-legend-item">
            <span
              className="purge-storage-legend-dot"
              style={{ backgroundColor: seg.color }}
            />
            <span className="purge-storage-legend-label">{seg.name}</span>
          </div>
        ))}
      </div>
    </div>
...
function CategoryGroup({
  ...
  return (
    <div className="purge-category-group">
      <div className="purge-category-header" onClick={() => setExpanded(!expanded)}>
        <ChevronRight
          size={14}
          className={`purge-category-chevron ${expanded ? "expanded" : ""}`}
        />
        <input
          type="checkbox"
        ...
        />
        <span className="purge-category-name">{type}</span>
        <span className="purge-category-meta">
          {items.length} item{items.length !== 1 ? "s" : ""} · {formatSize(groupSize)}
        </span>
        <span className="purge-category-color-dot" style={{ backgroundColor: color }} />
      </div>

      {expanded && (
        <div className="purge-category-items">
          {items.map((artifact: any) => (
            <label key={artifact.artifact_path} className="purge-artifact-row">
              ...
              />
              <Folder size={14} className="purge-artifact-icon" />
              <div className="purge-artifact-info">
                <div className="purge-artifact-project">
                  {artifact.project_name}
                </div>
                <div
                  className="purge-artifact-path clickable"
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
...
function QuickFilters({
  ...
  return (
    <div className="purge-quick-filters">
      <button
        className={`purge-filter-btn ${active === "all" ? "active" : ""}`}
        onClick={() => onChange("all")}
      >
        All
      </button>
      <button
        className={`purge-filter-btn ${active === "node_modules" ? "active" : ""}`}
        onClick={() => onChange("node_modules")}
      >
        All node_modules
        {counts.nodeModules > 0 && <span className="purge-filter-count">{counts.nodeModules}</span>}
      </button>
      <button
        className={`purge-filter-btn ${active === "large" ? "active" : ""}`}
        onClick={() => onChange("large")}
      >
        All &gt;500 MB
        {counts.large > 0 && <span className="purge-filter-count">{counts.large}</span>}
      </button>
    </div>
  );
}
  ...
  const toggleGroupAll = useCallback(
    (paths: string[], select: boolean) => {
      for (const p of paths) {
        const isSelected = usePurgeStore.getState().selectedPaths.has(p);
  ...
  if (artifacts.length === 0) {
    return (
      <div className="centered">
        ...
        <div className="purge-idle-title">Nothing to purge</div>
        <div className="purge-idle-desc">
          No build artifacts were found in this directory. Your projects are already clean.
        </div>
    ...
    <>
      ...
      <div className="purge-list-header">
        <div className="purge-list-summary">
          <span className="purge-list-title">Purge</span>
          <span className="purge-list-size">{formatSize(allSize)}</span>
          <span className="purge-list-context">
      ...
      {/* Storage Bar */}
      <StorageBar categories={storageBarCategories} />

      {/* Quick Filters */}
      <QuickFilters active={quickFilter} onChange={setQuickFilter} counts={filterCounts} />

      {/* Search + Sort */}
      <div className="purge-search-row">
        <div className="purge-search-box">
          <Search size={13} className="purge-search-icon" />
          <input
            type="text"
            className="purge-search-input"
            placeholder="Search projects..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
          />
        </div>
        <div className="purge-sort-controls">
          <span className="purge-sort-label">Sort</span>
          <button
            className={`purge-sort-btn ${sortMode === "size" ? "active" : ""}`}
            onClick={() => setSortMode("size")}
          >
            Size
          </button>
          <button
            className={`purge-sort-btn ${sortMode === "name" ? "active" : ""}`}
      ...
      <div className="module-footer">
        ...
        >
          Purge {selectedPaths.size > 0 ? formatSize(selectedSize) : ""}
        </button>
      </div>

      <DeleteConfirmDialog
        visible={showConfirm}
        title={`Purge ${selectedPaths.size} artifact${selectedPaths.size > 1 ? "s" : ""} (${formatSize(selectedSize)})?`}
        onConfirm={() => { setShowConfirm(false); purge(); }}
...
function Confetti({ active }: { active: boolean }) {
  ...
  return <canvas ref={canvasRef} className="purge-confetti" />;
}
...
function PurgingView() {
  ...
  const artifacts = usePurgeStore((s) => s.artifacts);
  const selectedPaths = usePurgeStore((s) => s.selectedPaths);
  const dismissDone = usePurgeStore((s) => s.dismissDone);
  ...
  return (
    <div className={`centered${isDone ? " purge-done" : ""}`}>
      ...
      <div className="purge-ring-wrap">
        ...
        >
          ...
          <circle
            ...
            className="purge-ring-fill"
            filter={isDone ? "url(#ring-glow)" : undefined}
          />
        </svg>
        {isDone ? (
          <Check size={32} strokeWidth={2.5} className="purge-ring-check" />
        ) : (
          <span className="purge-ring-percent">{percent}%</span>
        )}
      </div>

      {/* Status text */}
      <div className="purge-ring-freed">
        {progress ? formatSize(progress.bytes_freed) : "0 B"} reclaimed
      </div>

      <div className="purge-ring-current">
        {isDone
      ...
      {isDone && card && (
        <div className={`purge-equiv-card${showCard ? " visible" : ""}`}>
          <div className="purge-equiv-icon">
            {card.isMilestone ? <SsdIcon /> : <span className="purge-equiv-emoji">{card.emoji}</span>}
          </div>
          <div className="purge-equiv-text">
            <div className="purge-equiv-title">{card.title}</div>
            <div className="purge-equiv-desc">{card.description}</div>
          </div>
        </div>
      )}
      ...
      {isDone && breakdownChips.length > 0 && (
        <div className={`purge-breakdown-chips${showChips ? " visible" : ""}`}>
          {visibleChips.map((chip) => (
            <span key={chip.type} className="purge-breakdown-chip">
              {chip.count} {chip.type.toLowerCase()} · {formatSize(chip.size)}
            </span>
          ))}
          {hiddenCount > 0 && !chipsExpanded && (
            <button
              className="purge-breakdown-chip purge-breakdown-more"
      ...
      {isDone && (
        <button
          className={`btn purge-done-btn${showDone ? " visible" : ""}`}
          onClick={dismissDone}
        >
          Done
        </button>
...
export default function Purge() {
  ...
  return (
    <div className="purge-container">
      ...
      {phase === "list" && <ListView />}
      {(phase === "purging" || phase === "done") && <PurgingView />}
    </div>
  );
}
